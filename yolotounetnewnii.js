...
const App = () => {
  ...
  const [segmentCanvasUrl, setSegmentCanvasUrl] = useState(null);
  ...
  const handleFile = async (e) => {
    ...
    const [W, H, D] = header.dims.slice(1, 4);
    ...
    for (let z = 1; z < D - 1; z++) {
      ...
      drawSliceToCanvas(G, W, H);

      const input = await makeRGBInput(R, G, B, W, H);
      ...
      if (boxes.length) {
        ...
        const box = { x: Math.floor(x1), y: Math.floor(y1), w: Math.floor(x2 - x1), h: Math.floor(y2 - y1), score };
        setDetectionInfo(box);
        ...
        render16x16Overlay(resizedInput, segMask);
        break;
      }
    }
  };

  const makeRGBInput = (R, G, B, W, H) => new Promise(resolve => {
  ...
  const resizeTo16x16 = (cropped) => new Promise(resolve => {
    ...
    const resized = document.createElement('canvas');
    resized.width = 16; resized.height = 16;
  ...
  const drawSliceToCanvas = (slice, W, H) => {
    ...
    for (let i = 0; i < slice.length; i++) {
      ...
      img.data.set([val, val, val, 255], i * 4);
    }
    ctx.putImageData(img, 0, 0);
  };
  ...
  const render16x16Overlay = (original, mask) => {
    ...
    for (let i = 0; i < 16 * 16; i++) {
      const gray = Math.floor(original[i] * 255);
      if (mask[i] > 0.5) {
        img.data.set([255, 0, 0, 180], i * 4); // red overlay
      } else {
        img.data.set([gray, gray, gray, 255], i * 4);
      }
    }
    ctx.putImageData(img, 0, 0);
    ...
    uctx.drawImage(canvas, 0, 0, 160, 160);
    setSegmentCanvasUrl(upscale.toDataURL());
  };

  return (
    <div style={{ padding: 20 }}>
      <h2>YOLO + U-Net (.nii)</h2>
      <input type="file" accept=".nii,.nii.gz" onChange={handleFile} />
      <canvas ref={canvasRef} style={{ border: '1px solid black', marginTop: 10 }} />
      {detectionInfo && (
        <div style={{ marginTop: 10 }}>
          <h4>Detection:</h4>
          <p>x: {detectionInfo.x}, y: {detectionInfo.y}, w: {detectionInfo.w}, h: {detectionInfo.h}, confidence: {detectionInfo.score.toFixed(2)}</p>
        </div>
      )}
      {segmentCanvasUrl && (
        <div>
          <h4>Segmented 16×16 Output:</h4>
          <img src={segmentCanvasUrl} width={160} height={160} alt="segmented" style={{ imageRendering: 'pixelated' }} />
        </div>
      )}
    </div>
  );
